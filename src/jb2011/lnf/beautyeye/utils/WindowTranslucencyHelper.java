 ...
 */
package jb2011.lnf.beautyeye.utils;
...
import jb2011.lnf.beautyeye.utils.LogHelper;
import jb2011.lnf.beautyeye.utils.ReflectHelper;
...
//	}
	...
	public static boolean isTranslucencySupported()
	{
		...
		{
			//1.7.0及以后版本
			if(jb2011.lnf.beautyeye.utils.JVM.current().isOrLater(jb2011.lnf.beautyeye.utils.JVM.JDK1_7))
			{
				...
				isTranslucencySupported = ((Boolean)(jb2011.lnf.beautyeye.utils.ReflectHelper.invokeMethod(GraphicsDevice.class, gd, "isWindowTranslucencySupported"
						, new Class[]{_WindowTranslucency}
						, new Object[]{Enum.valueOf(_WindowTranslucency, "TRANSLUCENT")}))).booleanValue();
			}
			else if(jb2011.lnf.beautyeye.utils.JVM.current().isOrLater(jb2011.lnf.beautyeye.utils.JVM.JDK1_6))
			{
				...
				isTranslucencySupported = ((Boolean)(jb2011.lnf.beautyeye.utils.ReflectHelper.invokeMethod(Class.forName("com.sun.awt.AWTUtilities"), _WindowTranslucency, "isTranslucencySupported"
						, new Class[]{_WindowTranslucency}
						, new Object[]{Enum.valueOf(_WindowTranslucency, "TRANSLUCENT")}))).booleanValue();
			}
		}
		catch (Exception e)
		{
			if(BeautyEyeLNFHelper.debug)
				e.printStackTrace();
			jb2011.lnf.beautyeye.utils.LogHelper.debug("Exception at WindowTranslucencyHelper.isTranslucencySupported(),"+e.getMessage());
		}
...
//			com.sun.awt.AWTUtilities.setWindowOpacity(w, opacity);
			//1.6.0_u12及以后版本
			if(jb2011.lnf.beautyeye.utils.JVM.current().isOneDotSixUpdate12OrAfter())//JDK1_6_U10//为什么要到u12才支持？因u10里的窗口透明存在BUG 6750920
			{
				if(!isTranslucencySupported())
				{
					jb2011.lnf.beautyeye.utils.LogHelper.debug("Your OS can't supported translucency.");
					return;
				}
				
				//1.7.0及以后版本
				if(jb2011.lnf.beautyeye.utils.JVM.current().isOrLater(jb2011.lnf.beautyeye.utils.JVM.JDK1_7))
					jb2011.lnf.beautyeye.utils.ReflectHelper.invokeMethod(Window.class, w, "setOpacity"
							, new Class[]{float.class}, new Object[]{opacity});
				else
					jb2011.lnf.beautyeye.utils.ReflectHelper.invokeStaticMethod("com.sun.awt.AWTUtilities", "setWindowOpacity"
							, new Class[]{Window.class, float.class}, new Object[]{w, opacity});
			}
			else
				jb2011.lnf.beautyeye.utils.LogHelper.debug("您的JRE版本不支持每像素半透明(需jre1.6_u12及以上版本)，BeautyEye外观将不能达到最佳视觉效果哦.");
		}
		catch (Exception e)
		{
			if(BeautyEyeLNFHelper.debug)
				e.printStackTrace();
			jb2011.lnf.beautyeye.utils.LogHelper.debug("您的JRE版本不支持每像素半透明(需jre1.6_u12及以上版本)，BeautyEye外观将不能达到最佳视觉效果哦."+e.getMessage());
		}
	}
...
//			com.sun.awt.AWTUtilities.setWindowOpaque(w, opaque);
			...
			if(jb2011.lnf.beautyeye.utils.JVM.current().isOneDotSixUpdate12OrAfter())//JDK1_6_U10//为什么要到u12才支持？因u10里的窗口透明存在BUG 6750920
			{
				...
				//1.7.0及以后版本
				if(jb2011.lnf.beautyeye.utils.JVM.current().isOrLater(JVM.JDK1_7))
				{
//					if(isWindowTranslucencySupported())
				...
				jb2011.lnf.beautyeye.utils.LogHelper.debug("您的JRE版本不支持窗口透明(需jre1.6_u12及以上版本)，BeautyEye外观将不能达到最佳视觉效果哦.");
		}
		catch (Exception e)
		{
			if(BeautyEyeLNFHelper.debug)
